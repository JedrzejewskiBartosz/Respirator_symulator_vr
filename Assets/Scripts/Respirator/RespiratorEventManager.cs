...
public class RespiratorEventManager : MonoBehaviour
{
    ...
    void Awake()
    {
        // 1. Zabezpieczenie przed błędem rzucania obiektami kinematycznymi (Throw on Detach)
        foreach (var grab in GetComponentsInChildren<UnityEngine.XR.Interaction.Toolkit.Interactables.XRGrabInteractable>(true))
        {
        ...
        if (gniazdoRurki != null)
        {
            ...
            if (gniazdoRurki.startingSelectedInteractable is UnityEngine.XR.Interaction.Toolkit.Interactables.XRGrabInteractable pipeGrab)
            {
                pipeGrab.throwOnDetach = false;
            }
        }

        // 2. Generowanie przycisków na panelu przy starcie
    ...
    public void GenerujPrzyciskiNaPanelu()
    {
        ...
        if (template == null)
        {
            ...
            {
                // Szukamy dowolnego wcześniej wygenerowanego przycisku na panelu
        ...
        // Jeśli nadal brak na tym panelu, poszukaj w innych respiratorach
        if (template == null)
        {
            var allManagers = Object.FindObjectsByType<RespiratorEventManager>(FindObjectsSortMode.None);
            foreach (var mgr in allManagers)
            {
                if (mgr != this && mgr.buttonPrefab != null)
                {
                    template = mgr.buttonPrefab;
                    break;
                }
                if (mgr != this && mgr.buttonPanel != null)
                {
                    foreach (Transform child in mgr.buttonPanel)
                    {
                        if (child.name.StartsWith("Phisic_button"))
                        {
                            template = child.gameObject;
        ...
        if (template == null)
        {
            Debug.LogWarning($"[RespiratorEventManager] Brak szablonu Phisic_button lub buttonPrefab do generowania na {gameObject.name}.");
            return;
        }

        // Zapisujemy referencję do szablonu
        if (buttonPrefab == null)
        {
            buttonPrefab = template;
        }
        ...
        // 5. Zbieramy stare przyciski na panelu do usunięcia PO wygenerowaniu nowych
        ...
        GameObject firstCreatedBtn = null;
        ...
        for (int i = 0; i < buttonCount; i++)
        {
            ...
            if (firstCreatedBtn == null) firstCreatedBtn = btnInstance;
            ...
            pushScript.maxPushDepthMeters = 0.012f;
            pushScript.buttonRadiusMeters = 0.045f;
            pushScript.activationThreshold = 0.75f;
            pushScript.fullReleaseThreshold = 0.15f;
            pushScript.returnSpeed = 25f;
            pushScript.requireTriggerPressed = false;
            pushScript.maxApproachAngleDegrees = 50f;

            Transform plate = btnInstance.transform.Find("Button_plate") ?? btnInstance.transform.Find("button") ?? btnInstance.transform.GetChild(0);
            pushScript.buttonPlate = plate;
            pushScript.buttonFrame = btnInstance.transform.Find("Button_frame");
            ...
            if (plate != null)
            {
                ...
                {
                    mr.sharedMaterial = wybraneKolory[i].material;
                    pushScript.normalColor = wybraneKolory[i].material.color;
                }
            }
        ...
        // 7. Usuwamy stare przyciski (po wygenerowaniu nowych)
        ...
        if (buttonPrefab == null && firstCreatedBtn != null)
        {
            buttonPrefab = firstCreatedBtn;
        }
