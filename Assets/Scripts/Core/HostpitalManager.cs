...
{
    [Header("Stan Gry / Symulacji")]
    [Tooltip("Czy symulacja jest aktualnie aktywna (uruchomiona przyciskiem Start)")]
    public bool czySymulacjaAktywna = false;
    ...
    void Awake()
    {
        // Jeśli lista nie została ręcznie przypisana w Inspektorze, znajdź respiratory automatycznie
        if (respiratory == null || respiratory.Count == 0)
        {
    ...
    void Start()
    {
        ...
        CancelInvoke();
        ResetujTimer();
        ZresetujWszystkieAwarie();
        AktualizujZegarek();
    }

    void Update()
    {
        // Awarie losują się TYLKO i WYŁĄCZNIE wtedy, gdy gra została wystartowana przyciskiem
        if (czySymulacjaAktywna)
        {
    ...
    /// <summary>
    /// Uruchamia symulację szpitalną (wywoływane przez StartGameButton).
    /// </summary>
    public void RozpocznijSymulacje()
    {
        czySymulacjaAktywna = true;
        CancelInvoke();
        ResetujTimer();

        // Pierwsza awaria pojawia się po 2 sekundach od naciśnięcia Start
        Invoke(nameof(SprobujWywolacAwarie), 2.0f);

        Debug.Log("[HospitalManager] ▶ START SYMULACJI! Uruchomiono cykl losowania awarii.");
    }

    /// <summary>
    /// Zatrzymuje i resetuje całą symulację (wywoływane przez ponowne wciśnięcie StartGameButton).
    /// </summary>
    public void ZatrzymajIZresetujSymulacje()
    {
        czySymulacjaAktywna = false;
        CancelInvoke();
        ResetujTimer();
        ...
        Debug.Log("[HospitalManager] ⏹ RESET SYMULACJI! Zatrzymano losowanie awarii i wyczyszczono stan maszyn.");
    }

    /// <summary>
    /// Resetuje alarmy na wszystkich respiratorach.
    /// </summary>
    public void ZresetujWszystkieAwarie()
    {
    ...
    {
        if (respiratory == null || respiratory.Count == 0) return;
        ...
        Debug.Log($"[HospitalManager] ⚠ Wywołano awarię na {sprawneRespiratory[losowyIndeks].gameObject.name}!");
    }
    ...
    private void AktualizujZegarek()
    {
        ...
        for (int i = 0; i < respiratory.Count; i++)
        {
            ...
            else if (!czySymulacjaAktywna)
            {
                status = "OK (Czuwanie)";
                kolorTekstu = new Color(0.3f, 0.9f, 0.5f);
            }
